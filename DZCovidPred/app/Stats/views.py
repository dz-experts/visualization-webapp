...
def history():
    ...
    alg_info = req.json()['Algeria']
    
    cases = [ (x['date'] , x['confirmed']) for x in alg_info]
    deaths = [(x['date'] , x['deaths']) for x in alg_info]
    recovered = [ (x['date'] , x['recovered']) for x in alg_info]
...
def stillInfectedWilaya():
    ...
    infected = [ { 'name' : x['name'], 'name_ar' : x['name_ar'], 'active': x['active'] } for x in wilaya if x['active'] ]    

    # sort by the number of infected people
    infected = sorted(infected, key = lambda x: x['active'], reverse=False) 
